...
`server/env.ts` already loads the Anthropic settings before `harness/model.ts` reads them.
Keep it as the first server import: `harness/db.ts` will also read `DATABASE_URL` at import
time, and ES module imports run *before* top-level statements.
