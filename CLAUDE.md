...
- **LLM provider:** Anthropic-compatible Aixcoder relay via Vercel `@ai-sdk/anthropic` (model from `ANTHROPIC_MODEL`, example `claude-opus-5`).
...
harness/        the runtime — accretes one module per lesson
  ...
  model.ts      the AI SDK Anthropic model (ANTHROPIC_MODEL)
